...
void draw_bbox(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch, const std::vector<std::string>& class_names) {
    for (size_t i = 0; i < img_batch.size(); i++) {
        ...
        for (size_t j = 0; j < res.size(); j++) {
            ...
            int class_id = static_cast<int>(res[j].class_id);
            float confidence = res[j].conf; // Assuming `conf` holds the confidence score

            std::string label = (class_id >= 0 && class_id < class_names.size()) ? class_names[class_id] : "Unknown";
            label += " " + std::to_string(static_cast<int>(confidence * 100)) + "%"; // Format confidence as a percentage
            cv::putText(img, label, cv::Point(r.x, r.y - 5), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(255, 225, 0), 2);
        }
    }
}
